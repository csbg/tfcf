...
res.stats[, Control := ifelse(grepl("NonTargeting", Guide), "Yes", "No")]
res.stats[, padj := p.adjust(p, method = "BH")]

ggplot(res.stats, aes(y=z, x=paste(Genotype, "Control:", Control))) + 
...
for(libx in unique(res.stats$Library)){
  lDT <- res.stats[Library == libx]
  lDT <- lDT[Guide %in% lDT[padj < 0.05]$Guide]
  ggplot(lDT, aes(x=paste(Genotype, "Control:", Control), y=Guide, color=Score, size=-log10(padj))) + 
    geom_point() +
    scale_color_gradient2(low="blue", high="red") +
    facet_grid(. ~ Library + Date + Population1 + Population2, space = "free_x", scales = "free_x") + 
    theme_bw(12) +
    xlab("") +
    xRot()
