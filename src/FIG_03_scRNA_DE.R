...
umapMT <- FC.MT
umap.type.name <- "all"
gg <- if(umap.type == "top") unique(DE.RES[q_value < 0.05 & abs(estimate) > 1]$gene_id) else row.names(umapMT)
mt <- umapMT[gg,]
...
ggplot(pUMAP.de[guide %in% gg], aes(x=UMAP1, y=UMAP2)) +
  stat_summary_hex(
    ...
    fun=mean) +
  scale_fill_gradient2(high="#e31a1c",mid="#ffffff", low="#1f78b4") +
  facet_grid(tissue~guide) + theme_bw(12) +
  xlab("UMAP dimension 1") + ylab("UMAP dimension 2") +
  xlim(-dim.umap1,dim.umap1) + ylim(-dim.umap2,dim.umap2)
ggsave(out("RegulatoryMap_UMAP_",umap.type.name,"_Values_selection.pdf"), w=10,h=5, limitsize=FALSE)
