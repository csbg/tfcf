...
baseDir <- "POOLED_11_04_Replicates_NormFac_size_FromControls/"
out <- dirout(baseDir)
...
for(sysx in c("DM.CFSE", "DM.CD34")){
  ...
  ann.lib[, Genotype := NA]
  ann.lib[, Population := "Lib"]
  ann.lib[,System := sysx]
  ann <- rbind(ann, ann.lib)
}
ann[is.na(Population)]
ann <- ann[!is.na(Population)]
...
for(ai in 1:nrow(ann_replicates)){
  ...
  for(analysis.type in c("x")){
    ...
    # Design matrix -----------------------------------------------------------
    if(wtx){
      desMat <- model.matrix(~ Genotype * Population, data=data.frame(lAnn, row.names = lAnn$sample))
    } else {
      desMat <- model.matrix(~ Population, data=data.frame(lAnn, row.names = lAnn$sample))
    }
    ...
    lMT$samples <- normfacs.controls$samples
