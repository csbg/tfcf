...
out <- dirout("CITESEQ2_03_Analysis/")

data <- SCRNA.read_10Xh5.610(PATHS$CITESEQ2_CLEAN$DATA$matrix)
umap <- fread(PATHS$CITESEQ2_CLEAN$DATA$umap, check.names = TRUE)
clusters <- fread(PATHS$CITESEQ2_CLEAN$DATA$clusters, check.names = TRUE)
de.genes <- fread(PATHS$CITESEQ2_CLEAN$DATA$de, check.names = TRUE)
