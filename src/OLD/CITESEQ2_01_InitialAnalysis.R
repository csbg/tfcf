...
out <- dirout("CITESEQ2_01_InitialAnalysis/")

data <- SCRNA.read_10Xh5.610(PATHS$CITESEQ2$DATA$matrix)
umap <- fread(PATHS$CITESEQ2$DATA$umap, check.names = TRUE)
clusters <- fread(PATHS$CITESEQ2$DATA$clusters, check.names = TRUE)
de.genes <- fread(PATHS$CITESEQ2$DATA$de, check.names = TRUE)
