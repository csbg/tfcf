...
base.dir <- "SCRNA_41_03_GSEA_CellTypes/"
out <- dirout(base.dir)
...
# EnrichR -----------------------------------------------------------------
enr.terms <- fread(dirout_load('SCRNA_06_01_Markers')("Markers.used.tsv"))
enr.terms <- setNames(lapply(unique(enr.terms$db), function(dbx){
  with(enr.terms[db == dbx], split(value, L1))
}),unique(enr.terms$db))
