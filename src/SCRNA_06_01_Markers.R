...
# Dir ---------------------------------------------------------------------
basedir <- "SCRNA_06_01_Markers/"
out <- dirout(paste0(basedir))
...
tx <- "leukemia"
for(tx in PATHS$SCRNA$MONOCLE.NAMES){# loop through tissues
  
  # Load data for this tissue
  (load(PATHS$SCRNA$MONOCLE.DIR(tx)))
  outT <- dirout(paste0(basedir, tx, "/"))
