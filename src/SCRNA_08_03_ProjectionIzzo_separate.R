...
base.dir <- "SCRNA_08_03_ProjectionIzzo_separate/"
out <- dirout(base.dir)
...
for(tx in unique(xDT$tissue)){
  for(izzo.wtx in unique(xDT$dir)){
    inDT <- xDT[dir == izzo.wtx][tissue == tx]
    jMT <- jaccard.twolists(
      l1=with(inDT, split(rn, functional.cluster)), 
      l2=with(inDT, split(rn, labels))
    )
    jDT <- rbind(jDT, data.table(melt(data.table(jMT, keep.rownames = TRUE), id.vars = "rn"), tissue=tx, izzo.wt=izzo.wtx))
  }
}
ggplot(jDT, aes(x=rn, y=variable, fill=value)) + 
  theme_bw(12) + 
  geom_tile() + 
  facet_grid(izzo.wt ~ tissue) + 
  xRot() +
  scale_fill_gradient(low="white", high="blue")
ggsave(out("ComparisonToSingleR.pdf"), w=13,h=20)
