...
# ann$Cluster.number <- clusters[match(ann$rn, rn)]$functional.cluster
# x <- merge(
#   ann[guide == "NTC", .(NTC = .N), by=c("Cluster.number")][order(Cluster.number)],
#   ann[guide != "NTC", .(NTC = .N), by=c("Cluster.number")][order(Cluster.number)],
#   by=c("Cluster.number"), suffixes = c("_ntc", "_pert"), all=TRUE)
# x[, frac := NTC_ntc / NTC_pert]
