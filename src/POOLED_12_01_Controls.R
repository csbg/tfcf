...
for(ax in unique(res.ratios$Analysis)){
  ...
  if(nrow(resx) ==0) next
  x <- resx$Ratio
  stats <- rbind(stats, data.table(unique(resx[,-c("Ratio", "Guide"), with=F]), mean=mean(x), sd=sd(x)))
}
...
for(ax in unique(stats$Analysis)){
  x <- stats[Analysis == ax]
  stat.rnorm <- rbind(stat.rnorm, data.table(x, Ratio=rnorm(10000, mean=x$mean, sd=x$sd)))
}


ggplot(res.ratios, aes(x=Ratio)) + 
  geom_density(data=stat.rnorm, fill="#b2df8a", color=NA) +
  geom_density(aes(color=control)) +
  facet_grid(Group ~ Comparison, scales = "free") + 
  theme_bw(12)
...
res.stats[,log2Ratio := log2(Ratio)]
res.stats[, hit := sum(padj < 0.05) >= 2 & (all(sign(log2Ratio[padj < 0.05]) > 0) | all(sign(log2Ratio[padj < 0.05]) < 0)), by=c("Gene", "Analysis")]
